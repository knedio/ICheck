                ...
                <table id='datatables' class="table table-striped table-bordered" cellspacing="0" width="100%">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td><?php echo $pen->fname.' '.$pen->mname.' '.$pen->lname;?></td>
                      <td><?php echo $pen->user_no;?></td>
                      <td><?php echo date('F j, Y',strtotime($pen->date_submitted))?></td>
                      <td>
                        <a href="#viewrequest" data-toggle="modal" data-sid="<?php echo $pen->sched_id; ?>" data-timefrom="<?php echo $pen->time_from; ?>" data-timeto="<?php echo $pen->time_to; ?>" data-dp="<?php echo $pen->date; ?>" data-timeid="<?php echo $pen->timelogs_id; ?>" data-reqid="<?php echo $pen->request_id; ?>" data-userid="<?php echo $pen->user_id; ?>" data-name="<?php echo $pen->fname.' '.$pen->mname.' '.$pen->lname; ?>" data-datesubmitted="<?php echo date('F j, Y - g:i a',strtotime($pen->date_submitted)); ?>" data-sched="<?php echo date('g:i a',strtotime($pen->time_from)).' - '.date('g:i a',strtotime($pen->time_to)); ?>" data-date="<?php echo date('F j, Y',strtotime($pen->date)); ?>" data-status="<?php echo $pen->status; ?>" data-note="<?php echo $pen->requestor_note; ?>">
                        <span class="label label-default">View details</span></a>
                      </td>
                    </tr>
                    <?php $count++;} ?>
                  </tbody>
            ...
            <div class="table-responsive" style="padding-bottom:5em;">
                <table id='datatables2' class="table table-striped table-bordered" cellspacing="0" width="100%">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td><?php echo $count;?></td>
                      <td><?php echo $dec->fname.' '.$dec->mname.' '.$dec->lname;?></td>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="exampleModalLabel">View Pending Request!</h4>
      </div>
      <form method="POST" class="form-horizontal" action="<?php echo base_url();?>AdminController/updateReqStatus">
      <div class="modal-body ">
        ...
        <hr>
        <div class="form-group form-group-sm">
          <label class="col-sm-3 control-label">Leave Note : </label>
          <div class="col-sm-8">
            <textarea class="form-control" name="request_response" required></textarea>
          </div>
        </div>
        <div class="form-group form-group-sm text-center">
            <input type="hidden" name='request_id' ><input type="hidden" name='timelogs_id' >
            <input type="hidden" name='time_from' ><input type="hidden" name='time_to' >
            <input type="hidden" name='sched_id' ><input type="hidden" name='dp' >
            <input type="submit" class="btn btn-danger" name='stat' value="Decline">
            <input type="submit" class="btn btn-primary" name='stat' value="Approve">
        </div>
      </div>
      </form> 
